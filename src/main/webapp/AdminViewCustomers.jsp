...
<head>
    ...
    <style>
        ...
        * {
        ...
        .status-active {
            background: rgba(40, 167, 69, 0.15); /* Success light */
            color: var(--success-color);
        }

        .status-pending {
            background: rgba(255, 193, 7, 0.15); /* Warning light */
            color: var(--warning-color);
        }

        .status-blocked {
            background: rgba(220, 53, 69, 0.15); /* Danger light */
            color: var(--danger-color);
        }

        /* Actions Buttons (Refined Look) */
        .actions {
            display: flex;
    ...
    <main class="main-content">
        <header class="main-header">
            <div class="header-title">
                <h1><i class="fas fa-users" style="color: var(--primary-color);"></i> Customer Management</h1>
                <p>View and manage all registered customer profiles.</p>
            </div>
            ...
            <div class="table-card-header">
                <div>
                    ...
                    <span>
                        ...
                        <%= (customers != null ? customers.size() : 0) %> total registered customers
                    </span>
                </div>
            </div>
                        ...
                        if (customers != null && !customers.isEmpty()) {
                            for (Customer c : customers) {
                                ...
                                if ("Active".equalsIgnoreCase(status)) {
                                    css = "status-active";
                                } else if ("Inactive".equalsIgnoreCase(status) || "Blocked".equalsIgnoreCase(status)) {
                                    css = "status-blocked";
                                }
                    %>
                        <tr>
                            ...
                            <td>
                                <div class="actions">
                                    <button class="action-btn view"
        								onclick="window.location.href='<%=request.getContextPath()%>/admin/view-customer?cid=<%=c.getCid()%>'">
    									<i class="fas fa-eye"></i>
									</button>

                                    <button class="action-btn edit"
        								onclick="window.location.href='<%=request.getContextPath()%>/admin/edit-customer?cid=<%=c.getCid()%>'">
    									<i class="fas fa-edit"></i>
									</button>

                                    <button class="action-btn block"
        								title="Block / Unblock"
        								onclick="window.location.href='<%=request.getContextPath()%>/admin/toggle-block?cid=<%=c.getCid()%>&status=<%=c.getStatus()%>'">
    									<i class="fas fa-user-slash"></i>
									</button>

                                    <button class="action-btn delete"
        								onclick="if(confirm('Delete this customer?')) 
        								window.location.href='<%=request.getContextPath()%>/admin/delete-customer?cid=<%=c.getCid()%>'">
                    ...
                    %>
                        <tr>
                            <td colspan="7" style="text-align: center; color: var(--text-light); padding: 20px;">
                                <i class="fas fa-exclamation-circle"></i> No customers found in the database.
                            </td>
                        </tr>
                    <%
                        }
